...
function tvLobby() {
  ...
  io.to('tv').emit('tv_lobby', { joinUrl: lanUrl() + '/mobile.html', playerCount: game.players.size });
}

// Snapshot fuer TV-Clients, die mitten im Spiel verbinden (Cast-Receiver!).
// Enthaelt NIE correctAnswer vor dem Reveal.
function tvSnapshot() {
  const base = { state: game.state, joinUrl: lanUrl() + '/mobile.html', playerCount: game.players.size };
  if (game.state === STATES.QUESTION) {
    const q = questions[game.currentIndex];
    return { ...base, question: publicQuestion(q), index: game.currentIndex,
      total: questions.length, endsAt: game.questionEndsAt, serverNow: Date.now(),
      answerCount: game.answers.size };
  }
  if (game.state === STATES.REVEAL) {
    const q = questions[game.currentIndex];
    return { ...base, question: publicQuestion(q), index: game.currentIndex,
      total: questions.length, correctIndex: q.correctAnswer, ranking: ranking() };
  }
  if (game.state === STATES.FINISHED) return { ...base, ranking: ranking() };
  return base;
}
...
function startQuestion(index) {
  ...
  // 3) Frage an Handys UND HTML-TVs erst nach STREAM_DELAY — synchron zum Video.
  //    (Bei HTML-TV ohne Video-Stream: STREAM_DELAY=0 setzen, dann ist alles sofort.)
  game.timers.push(setTimeout(() => {
...
function revealQuestion() {
  ...
  io.to('tv').emit('tv_reveal', { correctIndex: q.correctAnswer, ranking: ranking() });
  pushHostState();
}
...
function finishQuiz() {
  ...
  io.to('tv').emit('tv_finished', { ranking: finalRanking });
  pushHostState();
}
...
io.on('connection', socket => {
  ...
  // TV-Clients: tv.html (Browser/Tab-Cast) und receiver.html (Custom Cast Receiver)
  socket.on('tv_join', () => {
    socket.join('tv');
    socket.emit('tv_state', tvSnapshot());
  });
  ...
  socket.on('answer', (data, cb) => {
    ...
    io.to('tv').emit('tv_answers', { answerCount: game.answers.size, playerCount: game.players.size });
    pushHostState();
    cb && cb({ ok: true });
  });

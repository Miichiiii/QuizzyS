...
async function main() {
  ...
  watchForLeaks(p1, 'p1');
  watchForLeaks(p2, 'p2');
  // TV-Kanal: correctIndex darf NUR in tv_reveal / tv_finished / tv_state(REVEAL) stecken.
  tv.onAny((event, ...args) => {
    if (event === 'tv_reveal' || event === 'tv_finished') return;
    const json = JSON.stringify(args);
    if (event === 'tv_state' && /"state":"REVEAL"/.test(json)) return;
    if (/correctAnswer|correctIndex/.test(json)) {
      failed++;
      console.log('  FAIL  LEAK bei tv / ' + event + ': ' + json);
    }
  });
  ...
  // 2b) TV-Client: Snapshot beim Join (Lobby)
  const tvSnap = await new Promise(r => { tv.once('tv_state', r); tv.emit('tv_join'); });
  assert(tvSnap.state === 'LOBBY' && typeof tvSnap.joinUrl === 'string',
    'TV-Client bekommt Lobby-Snapshot beim Join');

  // 3) Frage-Events sammeln
  let q1p1 = null, q1ReceivedAt = 0;
  p1.on('question_start', q => { if (!q1p1) { q1p1 = q; q1ReceivedAt = Date.now(); } });
  let tvQ = null, tvQAt = 0, tvReveal = null;
  tv.on('tv_question', q => { if (!tvQ) { tvQ = q; tvQAt = Date.now(); } });
  tv.once('tv_reveal', r => { tvReveal = r; });
  ...
  assert(tvQ !== null && tvQ.question.questionId === 'q1', 'tv_question kommt beim TV-Client an');
  assert(tvQ && !('correctAnswer' in tvQ.question), 'tv_question enthaelt KEINE Loesung');
  assert(Math.abs(tvQAt - q1ReceivedAt) < 200, 'TV und Handy bekommen die Frage synchron');
  ...
  await sleep(150); // tv_reveal kommt ueber separaten Socket
  assert(tvReveal !== null && tvReveal.correctIndex === 1, 'tv_reveal liefert Loesung erst zur Aufloesung');
